...
import {
  Image, Linking, Platform, Pressable, ScrollView,
  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.hero}>
          ...
          <View style={s.heroInfo}>
            ...
            <View style={s.heroMeta}>
              ...
              <Text style={[s.heroLocation, { fontSize: fonts.xs }]}>{lugar.ubicacion}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={s.mainCard}>
          <View style={s.metaRow}>
            <View style={s.ratingWrap}>
              {[1,2,3,4,5].map(star => (
                <Ionicons key={star} name={star <= Math.round(lugar.rating ?? 5) ? 'star' : 'star-outline'} size={16} color="#FFD700" />
              ))}
              <Text style={[s.ratingVal, { fontSize: fonts.sm }]}>{lugar.rating ?? '5.0'}</Text>
            </View>
            <View style={s.priceBadge}>
              <Text style={[s.priceText, { fontSize: fonts.sm }]}>{getCostoTexto(lugar.costo)}</Text>
            </View>
          </View>

          <View style={s.actionRow}>
            <Pressable style={({ pressed }) => [s.actionBtn, { opacity: pressed ? 0.88 : 1 }]} onPress={abrirMapa}>
            ...
            <Pressable style={({ pressed }) => [s.shareBtn, { opacity: pressed ? 0.85 : 1 }]} onPress={compartir}>
        ...
        <View style={s.tabs}>
          <Pressable style={[s.tab, activeTab === 'info' && s.tabActive]} onPress={() => setActiveTab('info')}>
            <Ionicons name="information-circle-outline" size={16} color={activeTab === 'info' ? '#E96928' : colors.subtext} />
            <Text style={[s.tabText, { fontSize: fonts.sm }, activeTab === 'info' && s.tabTextActive]}>{t('detail_info')}</Text>
          </Pressable>
          <Pressable style={[s.tab, activeTab === 'reseñas' && s.tabActive]} onPress={() => setActiveTab('reseñas')}>
            <Ionicons name="chatbubbles-outline" size={16} color={activeTab === 'reseñas' ? '#E96928' : colors.subtext} />
            <Text style={[s.tabText, { fontSize: fonts.sm }, activeTab === 'reseñas' && s.tabTextActive]}>{t('detail_reviews')}</Text>
          </Pressable>
        </View>
          ...
          <View style={s.detailsCard}>
            ...
            <View style={s.detailRow}>
              ...
              <View style={s.detailInfo}>
                ...
                <Text style={[s.detailValue, { fontSize: fonts.sm }]}>{lugar.ubicacion}</Text>
              </View>
            ...
            <View style={s.detailRow}>
              <View style={[s.detailIconWrap, { backgroundColor: 'rgba(245,190,65,0.12)' }]}>
                <MaterialCommunityIcons name="tag-outline" size={20} color="#F5BE41" />
              </View>
              <View style={s.detailInfo}>
                <Text style={[s.detailLabel, { fontSize: fonts.xs }]}>{t('detail_category')}</Text>
                <Text style={[s.detailValue, { fontSize: fonts.sm }]}>{getCategoriaTexto(lugar.categoria)}</Text>
              </View>
            </View>
            ...
            <View style={s.detailRow}>
              <View style={[s.detailIconWrap, { backgroundColor: 'rgba(233,105,40,0.12)' }]}>
                <MaterialCommunityIcons name="currency-usd" size={20} color="#E96928" />
              </View>
              <View style={s.detailInfo}>
...
const makeStyles = (c: any, f: any, isDark: boolean) => StyleSheet.create({
  ...
  emptyIconWrap:{ width: 90, height: 90, borderRadius: 28, justifyContent: 'center', alignItems: 'center', marginBottom: 16, shadowColor: '#E96928', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 8 },
  emptyTitle:   { fontWeight: '900', color: c.text, marginBottom: 20 },
  emptyBtn:     { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#E96928', paddingHorizontal: 22, paddingVertical: 12, borderRadius: 20, elevation: 4, shadowColor: '#E96928', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 8 },
  emptyBtnText: { color: '#fff', fontWeight: '800' },
  hero:         { width: '100%', height: 300 },
  heroImage:    { width: '100%', height: '100%' },
  backBtn: { position: 'absolute', top: Platform.OS === 'ios' ? 56 : 40, left: 18, backgroundColor: 'rgba(0,0,0,0.4)', width: 40, height: 40, borderRadius: 13, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)' },
  favBtn:  { position: 'absolute', top: Platform.OS === 'ios' ? 56 : 40, right: 18, backgroundColor: 'rgba(0,0,0,0.4)', width: 40, height: 40, borderRadius: 13, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)' },
  ...
  mainCard: { backgroundColor: c.card, marginTop: -22, borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingHorizontal: 22, paddingTop: 24, paddingBottom: 20, borderWidth: 1, borderColor: c.border, elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.08, shadowRadius: 8 },
  ...
  shareBtn:     { flex: 1, height: 50, borderWidth: 2, borderColor: '#E96928', borderRadius: 14, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 6 },
