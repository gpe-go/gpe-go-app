...
import {
  ...
  Image, ScrollView, Linking, Alert, Platform, StatusBar,
} from 'react-native';
...
export default function ExplorarScreen() {
  ...
  const obtenerUbicacion = async () => {
    ...
    const nuevaRegion = { latitude: loc.coords.latitude, longitude: loc.coords.longitude, latitudeDelta: 0.02, longitudeDelta: 0.02 };
  ...
  const Header = () => (
    <View>
      <View style={s.orangeBanner}>
        <Text style={s.bannerTitle}>{t('places')}</Text>
        <Text style={s.bannerSub}>{t('ST')}</Text>
          ...
          <View style={s.searchResults}>
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map(item => (
                <Pressable key={item.id} style={s.searchItem} onPress={() => openInMaps(item.nombre, item.ubicacion)}>
      ...
      <View style={s.section}>
        ...
        <Text style={[s.sectionTitle, { marginTop: 18 }]}>{t('categories')}</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 16 }}>
          {CATEGORIAS.map(cat => {
            ...
            return (
              <Pressable key={cat.id} onPress={() => filtrarCategoria(cat.value)}
                style={[s.categoryCard, activa && { borderColor: cat.color, borderWidth: 2 }]}>
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Pressable style={s.heartBadge} onPress={() => toggleFavorito({ ...item, origen: 'detalle' })}>
              <Ionicons name={esFavorito(item.id) ? 'heart' : 'heart-outline'} size={18} color={esFavorito(item.id) ? '#E11D48' : '#fff'} />
            </Pressable>
            <View style={s.cardInfo}>
              <Text style={s.placeName} numberOfLines={1}>{item.nombre}</Text>
              <Text style={s.placeSub}>{item.ubicacion}</Text>
              <Text style={s.placeCost}>{item.costo}</Text>
              <Pressable style={s.mapBtn} onPress={() => openInMaps(item.nombre, item.ubicacion)}>
                <Ionicons name="navigate" size={14} color="#fff" />
                <Text style={s.mapBtnText}>{t('location')}</Text>
              </Pressable>
            </View>
          </Pressable>
        )}
...
const makeStyles = (c: any, f: any) => StyleSheet.create({
  orangeBanner: { backgroundColor: '#E96928', paddingHorizontal: 20, paddingTop: Platform.OS === 'ios' ? 40 : 30, paddingBottom: 18, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
  bannerTitle:  { color: '#fff', fontSize: f.xl, fontWeight: '900' },
  bannerSub:    { color: '#fff', fontSize: f.sm, opacity: 0.9, marginBottom: 10 },

  searchBox:       { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 12, paddingHorizontal: 14, alignItems: 'center', height: 44 },
  ...
  sectionTitle:    { fontSize: f.md, fontWeight: 'bold', color: c.text },
  sectionTitleAlt: { fontSize: f.md, fontWeight: 'bold', marginLeft: 20, marginTop: 16, marginBottom: 10, color: c.text },

  mapBox:      { height: 170, borderRadius: 22, overflow: 'hidden', marginTop: 12, borderLeftWidth: 6, borderLeftColor: '#E96928', backgroundColor: c.card },
  locationBtn: { position: 'absolute', top: 10, right: 10, backgroundColor: c.card, width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },

  categoryCard: { backgroundColor: c.card, paddingVertical: 14, paddingHorizontal: 10, borderRadius: 16, marginRight: 12, alignItems: 'center', width: 105, marginTop: 12 },
  iconCircle:   { width: 46, height: 46, borderRadius: 23, justifyContent: 'center', alignItems: 'center', marginBottom: 6 },
  catName:      { fontWeight: '600', fontSize: f.xs, textAlign: 'center', color: c.text },

  placeCard:  { width: '44%', backgroundColor: c.card, margin: '3%', borderRadius: 18, overflow: 'hidden' },
  ...
  cardInfo:   { padding: 12 },
  placeName:  { fontWeight: 'bold', fontSize: f.sm, color: c.text },
  placeSub:   { fontSize: f.xs, color: c.subtext },
  placeCost:  { fontSize: f.xs, fontWeight: 'bold', marginTop: 4, color: '#E96928' },
  mapBtn:     { backgroundColor: '#E96928', marginTop: 8, padding: 7, borderRadius: 8, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 5 },
  mapBtnText: { color: '#fff', fontSize: f.xs, fontWeight: 'bold' },
});
