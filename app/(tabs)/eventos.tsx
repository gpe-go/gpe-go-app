...
const SUB_KEYS: Record<string, string> = {
  'Fútbol': 'sub_futbol', 'Atletismo': 'sub_atletismo', 'Básquetbol': 'sub_basquetbol',
  'Festivales': 'sub_festivales', 'Exposiciones': 'sub_exposiciones', 'Museos': 'cat_museos',
  'Ferias': 'sub_ferias', 'Degustaciones': 'sub_degustaciones', 'Comunitario': 'sub_comunitario',
  'Festival': 'sub_festival', 'Mundial 2026': 'sub_mundial',
};
...
const EventCard = React.memo(({ item, colors, fonts, onPress, t }: any) => {
  ...
  return (
    <Pressable style={({ pressed }) => [s.card, { opacity: pressed ? 0.93 : 1 }]} onPress={onPress}>
      <View style={s.cardImgWrapper}>
        ...
        {catInfo && (
          <View style={[s.catImgBadge, { backgroundColor: catInfo.color }]}>
            ...
            <Text style={s.catImgBadgeText}>{SUB_KEYS[item.sub] ? t(SUB_KEYS[item.sub]) : item.sub}</Text>
          </View>
        )}
      </View>
      <View style={s.cardContent}>
        <Text style={[s.eventTitle, { fontSize: fonts.base }]} numberOfLines={1}>{item.titulo}</Text>
...
export default function EventosScreen() {
  ...
  const eventoPrincipal = eventos.find((e: any) => e.especial);
  ...
  const fetchData = useCallback(async () => { await refreshEventos(); }, [refreshEventos]);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={{ alignItems: 'center', marginTop: 40 }}>
            ...
            <Text style={{ color: '#94a3b8', marginTop: 12 }}>{t('events_no_results')}</Text>
          </View>
        }
        ListHeaderComponent={
          <View>
              ...
              <View style={{ position: 'relative', zIndex: 100, marginTop: 16 }}>
                ...
                {search.length > 0 && (
                  <View style={s.searchResults}>
                    ...
                    {eventos.filter((e: any) => e.titulo.toLowerCase().includes(search.toLowerCase())).map((item: any) => (
            ...
            <View style={s.catSection}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                ...
                >
                  ...
                  <Text style={[s.catText, { fontSize: fonts.xs }, activeCat === 'Todas' && s.catTextActive]}>{t('all')}</Text>
                </Pressable>
                {CATEGORIAS.map(cat => {
                  const activa = activeCat === cat.value;
                  return (
                    <Pressable key={cat.id} style={[s.catItem, activa && { backgroundColor: cat.color, borderColor: cat.color }]} onPress={() => setActiveCat(cat.value)}>
                      <MaterialCommunityIcons name={cat.icon as any} size={16} color={activa ? '#fff' : colors.subtext} />
                      <Text style={[s.catText, { fontSize: fonts.xs }, activa && s.catTextActive]}>{t(cat.labelKey)}</Text>
                    </Pressable>
                  );
                })}
              </ScrollView>
            ...
            {eventoPrincipal && (
              <View style={s.featuredSection}>
                <View style={s.sectionHeader}>
                  <View style={s.sectionDot} />
                  <Text style={[s.sectionTitle, { fontSize: fonts.md }]}>{t('events_featured')}</Text>
                </View>
                <TouchableWithoutFeedback onPressIn={startVideo} onPressOut={stopVideo}>
                  <View style={s.mainEventCard}>
                    {!playVideo ? (
                      ...
                      <VideoView style={s.mainEventImage} player={player} allowsPictureInPicture={false} nativeControls={false} />
                    )}
                    ...
                    <View style={s.holdHint}>
                      ...
                      <Text style={[s.holdHintText, { fontSize: fonts.xs }]}>{t('events_hold_video')}</Text>
                    </View>
                    <View style={s.mainEventOverlay}>
                      <Text style={[s.mainEventTitle, { fontSize: fonts.xl }]} numberOfLines={2}>{eventoPrincipal.titulo}</Text>
...
const makeStyles = (c: any, f: any) => StyleSheet.create({
  ...
  catItem: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: c.card, paddingHorizontal: 14, height: 38, borderRadius: 20, marginRight: 10, borderWidth: 1.5, borderColor: c.border, elevation: 2 },
  catText: { fontWeight: '600', color: c.subtext },
  catTextActive: { color: '#fff', fontWeight: '700' },
  featuredSection: { marginTop: 20 },
  mainEventCard: { marginHorizontal: 20, marginBottom: 8, borderRadius: 24, overflow: 'hidden', backgroundColor: '#000', elevation: 10 },
  ...
  card: { backgroundColor: c.card, marginHorizontal: 20, marginBottom: 12, borderRadius: 20, overflow: 'hidden', flexDirection: 'row', borderWidth: 1, borderColor: c.border, elevation: 3 },
